...
from Dataset_notest import SumDataset
import os
from tqdm import tqdm
from Model_notest import *
import numpy as np
...
NlLen_map = {"Time":400, "Math":400, "Lang":100, "Chart": 1000, "Mockito":500, "unknown":2200, "Closure":320}
CodeLen_map = {"Time":1300, "Math":2700, "Lang":300, "Chart":6000, "Mockito":1176, "unknown":2800, "Closure":2000}
...
args = dotdict({
    ...
    'SentenceLen':10,
    'batch_size':batch_size_map[sys.argv[2]],
    'embedding_size':32,
    'WoLen':15,
    'Vocsize':100,
    'Nl_Vocsize':100,
            ...
            if index == 0:
                ...
                for k, devBatch in tqdm(enumerate(val_set.Get_Train(len(val_set)))):
                        for i in range(len(devBatch)):
                            devBatch[i] = gVar(devBatch[i])
                        with torch.no_grad():
                            l, pre, xx = model(devBatch[0], devBatch[1], devBatch[2], devBatch[3], devBatch[4], devBatch[5], devBatch[6], devBatch[7], devBatch[8])
                            resmask = torch.eq(devBatch[0], 2)
                            s = -pre#-pre[:, :, 1]
                            s = s.masked_fill(resmask == 0, 1e9)
                            pred = s.argsort(dim=-1)
                        ...
                        if epoch == 10:
                            ...
                            score_path = os.path.join('spectrum_scores_notest_final', p, "%s_%s_%s"%(args.lr, args.seed, args.batch_size))
...
if __name__ == "__main__":
    ...
    open('%sres%d_notest_final.pkl'%(p, int(sys.argv[1])), 'wb').write(pickle.dumps(res))
    '''res = {}
    for i in range(100):
        a = train(i)
        res[i] = a 
