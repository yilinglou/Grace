...
class NlEncoder(nn.Module):
    def __init__(self, args):
        ...
        self.token_embedding = nn.Embedding(args.Nl_Vocsize, self.embedding_size - 3)
    ...
    def forward(self, input_node, failnum, passnum, inputad, res, inputtext, linenode, lfailnum, lpassnum):
        ...
        nodeem = torch.cat([nodeem, inputtext.unsqueeze(-1).float(), failnum.unsqueeze(-1).float(), passnum.unsqueeze(-1).float()], dim=-1)
        ...
        lineem = torch.cat([lineem, lfailnum.unsqueeze(-1).float(), lpassnum.unsqueeze(-1).float()], dim=-1)
