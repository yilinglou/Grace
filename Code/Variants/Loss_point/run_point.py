            ...
            if index == 0:
                ...
                for k, devBatch in tqdm(enumerate(val_set.Get_Train(len(val_set)))):
                        ...
                        if epoch == 10:
                            ...
                            score_path = os.path.join('spectrum_scores_point_final', p, "%s_%s_%s"%(args.lr, args.seed, args.batch_size))
...
if __name__ == "__main__":
    ...
    open('%sres%d_point_final.pkl'%(p, int(sys.argv[1])), 'wb').write(pickle.dumps(res))
    '''res = {}
    for i in range(100):
        a = train(i)
        res[i] = a 
