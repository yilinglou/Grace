...
def train(t = 5, p='Math'):
    ...
    for epoch in range(11):
        ...
        for dBatch in tqdm(train_set.Get_Train(args.batch_size)):
            if index == 0:
                ...
                for k, devBatch in tqdm(enumerate(val_set.Get_Train(len(val_set)))):
                        for i in range(len(devBatch)):
                            devBatch[i] = gVar(devBatch[i])
                        with torch.no_grad():
                            l, pre, xx = model(devBatch[0], devBatch[1], devBatch[2], devBatch[3], devBatch[4], devBatch[5], devBatch[6], devBatch[7], 'val')
                            resmask = torch.eq(devBatch[0], 2)
                            s = -pre#-pre[:, :, 1]
                            s = s.masked_fill(resmask == 0, 1e9)
                            pred = s.argsort(dim=-1)
                        ...
                        if epoch == 10:
                            ...
                            score_path = os.path.join('spectrum_scores_pair_final', p, "%s_%s_%s"%(args.lr, args.seed, args.batch_size))
            ...
            loss, _, _ = model(dBatch[0], dBatch[1], dBatch[2], dBatch[3], dBatch[4], dBatch[5], dBatch[6], dBatch[7], 'train')
            print(loss.item())
            optimizer.zero_grad()
            loss.backward()
            #print(loss.item())
...
if __name__ == "__main__":
    ...
    open('%sres%d_pair_final.pkl'%(p, int(sys.argv[1])), 'wb').write(pickle.dumps(res))
    '''res = {}
    for i in range(100):
        a = train(i)
        res[i] = a 
