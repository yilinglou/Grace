...
class NlEncoder(nn.Module):
    ...
    def forward(self, input_node, inputtype, inputad, res, inputtext, linenode, linetype, linemus, typee):
        ...
        if typee == 'train':
            for k in range(len(xx)):
                bug = []
                nobug = []
                for i in range(len(res[k][:resmask.sum(-1)[k].item()])):
                    if res[k][i] == 1:
                        bug.append(i)
                    else:
                        nobug.append(i)
                ...
                assert len(bug) > 0
                if len(nobug) == 0:
                    cur_loss = -torch.log(resSoftmax[k].clamp(min=1e-10, max=1)) * res[k]
                    ...
                    cur_loss = 0
                    cur_num = len(bug) * len(nobug)
                    for i in range(len(bug)):
                        for j in range(len(nobug)):
                            cur_loss += F.relu(0.6 - (xx[k][bug[i]] - xx[k][nobug[j]]))
                    loss += cur_loss / cur_num
                    # print(2,cur_loss / cur_num)
            loss = loss / len(xx)
        # loss = F.binary_cross_entropy_with_logits(xx, res.float(), reduction = 'none')
